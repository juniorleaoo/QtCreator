...
void MainWindow::on_pushButtonCriarMatA_clicked()
{
    try {
        ...
        int coluna = ui->lineEditQuantidadeDeColunasMatA->text().toInt();
                                //
        matA = new TP2::Matriz<int>(linha,coluna); /*acessei a namespace TP2 e acionei o construtor Matriz e passei a linha e coluna como parâmetro */
        for(int l=0; l<linha;l++){
            for(int c=0; c < coluna; c++){
                 //                                                                                                                     //
                int elemento = QInputDialog::getText(this, "Leitura","Matriz A ["+QString::number(l)+","+QString::number(c) + "] = ").toInt();
        ...
        //TRIANGULAR SUPERIOR
        QString saida3;
        if(matA->matrizTringSuperior()==true)
            saida3 = "Sim";
        else
            saida3 = "Não";
...
void MainWindow::on_pushButtonCriarMatB_clicked()
{
    try {
        ...
        for(int l=0; l<linha;l++){
            for(int c=0; c < coluna; c++){
                 //
                int elemento = QInputDialog::getText(this, "Leitura",
                                                    "Matriz B ["+QString::number(l)+","+
                                                    QString::number(c) + "] = ").toInt();
...
void MainWindow::on_pushButtonAdicionar_clicked()
{
    try{
        ...
        if(matB==0) throw QString("Operacao de + nao pode ser executada");
                    //
        TP2::Matriz<int> *matC = (*matA)+matB; //pra quem o matA aponta e soma com o matB, o "+" tem como parâmetro um ponteiro, por isso ele não usa o (*matB)
    ...
    if(aux2 == "A"){
        try{
            if(matA==0) throw QString("Operacao de 'Transposta' nao pode ser executada");
                        //
            TP2::Matriz<int> *matC = matA->matrizTransp();
            ui->textEditSaidaResultado->setText(matC->getMatriz());
            if(matC) delete matC;
        }catch(QString erro){
            QMessageBox::information(this,"erro",erro);
    ...
    if(aux2 == "B"){
        try{
            if(matB==0) throw QString("Operacao de 'Transposta' nao pode ser executada");
                        //
            TP2::Matriz<int> *matC = matB->matrizTransp();
            ui->textEditSaidaResultado->setText(matC->getMatriz());
            if(matC) delete matC;
        }catch(QString erro){
            QMessageBox::information(this,"erro",erro);
...
void MainWindow::on_pushButtonMultiplicarK_clicked()
{
    ...
    if(aux2 == "A"){
        try{
            if(matA==0) throw QString("Operacao 'Multiplicar K' nao pode ser executada");
            //                              //
            int elemento = QInputDialog::getInt(this, "Leitura","K =");
                        //
            TP2::Matriz<int> *matC = matA->multiplicarK(elemento);
            ui->textEditSaidaResultado->setText(matC->getMatriz());
            if(matC) delete matC;
        }catch(QString &erro){
            QMessageBox::information(this, "ERRO NO SISTEMA",erro);
    ...
    if(aux2 == "B"){
        try{
            if(matB==0) throw QString("Operacao 'Multiplicar K' nao pode ser executada");
            //                              //
            int elemento = QInputDialog::getInt(this, "Leitura","K =");
                        //
            TP2::Matriz<int> *matC = matB->multiplicarK(elemento);
            ui->textEditSaidaResultado->setText(matC->getMatriz());
            if(matC) delete matC;
        }catch(QString &erro){
            QMessageBox::information(this, "ERRO NO SISTEMA",erro);
...
void MainWindow::on_pushButtonPotencia_clicked()
{
    ...
    if(aux2 == "A"){
    try{
        if(matA==0) throw QString("Operacao 'Potencia' nao pode ser executada");
        //                              //
        int elemento = QInputDialog::getInt(this, "Potencia","N =");
                    //
        TP2::Matriz<int> *matC = matA->potencia(elemento);
        ui->textEditSaidaResultado->setText(matC->getMatriz());
        if(matC) delete matC;
    }catch(QString &erro){
        QMessageBox::information(this, "ERRO NO SISTEMA",erro);
    ...
    if(aux2 == "B"){
        try{
            if(matB==0) throw QString("Operacao 'Potencia' nao pode ser executada");
            //                              //
            int elemento = QInputDialog::getInt(this, "Potencia","N =");
                        //
            TP2::Matriz<int> *matC = matB->potencia(elemento);
            ui->textEditSaidaResultado->setText(matC->getMatriz());
            if(matC) delete matC;
        }catch(QString &erro){
            QMessageBox::information(this, "ERRO NO SISTEMA",erro);
